...
<div class="row mb-5">
    ...
    <div class="col-6">
        <div class="mb-3">
            ...
            @enderror"
                id="square_meters" name="square_meters" value="{{ old('square_meters', $apartment->square_meters) }}">
            @error('title')
                <div class="invalid-feedback">
                    {{ $message }}
                </div>
        ...
        <div class="mt-3">
            @foreach ($services as $service)
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="checkbox" id="{{ "tech-$service->label" }}" name='services[]'
                        value="{{ $service->id }}" @if (in_array($service->id, old('services', $prev_service ?? []))) checked @endif>
                    <label class="form-check-label" for="{{ "tech-$service->label" }}">{{ $service->label }}<i class="{{$service->icon}} mx-2"></i></label>
                </div>
            @endforeach
        </div>
        @error('technologies')
    ...
    <div class="col-11">
        <div class="my-3">
            ...
            <div @class([
                ...
                <button class="btn btn-outline-secondary w-25 me-1" type="button" id="change-cover_image-button">Cambia
                    Immagine</button>
                <input type="text" class="form-control" value="{{ old('cover_image', $apartment->cover_image) }}"
                    disabled>
            </div>
            <input type="file"
    ...
    <div class="col-1 d-flex justify-content-center align-items-center">
        <div>
            <img src="{{ old('cover_image', $apartment->cover_image) ? $apartment->printImage() : 'https://marcolanci.it/boolean/assets/placeholder.png' }}"
                class="img-fluid" alt="{{ $apartment->cover_image ? $apartment->title : 'preview' }}" id='preview'>
        </div>
    </div>
    <div class="col-6">
        <div class="my-3">
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" role="switch" id="is_visible" name="is_visible" @if (old('is_visible', $apartment->is_visible)) checked @endif>
